class WinemakersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @winemaker.save
        format.html { redirect_to winemaker_url(@winemaker), notice: "Winemaker was successfully created." }
  ...
  def update
    respond_to do |format|
      if @winemaker.update(winemaker_params)
        format.html { redirect_to winemaker_url(@winemaker), notice: "Winemaker was successfully updated." }
  ...
  private
    ...
    def winemaker_params
      params.require(:winemaker).permit(:name, :age, :nationality, :record, :position, :magazine, {winetasted_ids: []})
    end
end
